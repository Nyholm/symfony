...
class AmazonSnsReceiverTest extends TestCase
{
    public function testItReturnsTheDecodedMessageToTheHandler()
    {
        $serializer = $this->createSerializer();

        $snsEnvelop = $this->createSnsEnvelope();
        $connection = $this->createMock(Connection::class);
        $connection->method('get')->willReturn($snsEnvelop);

        $receiver = new AmazonSnsReceiver($connection, $serializer);
    ...
    private function createSnsEnvelope()
    {
        return [
            'id' => 1,
            'body' => '{"message": "Hi"}',
